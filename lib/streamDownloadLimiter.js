...
module.exports.stream = function(filePath,res,timeKB){
    ...
    stream.on('finish', function () { console.log("finish"); });
    // const stream = readStream.pipe(res);
}

module.exports.streamSleep = function(filePath,res,stopTime){
    const stat = fs.statSync(filePath);
    const contentType = mime.lookup(filePath);
    let data = '';
    res.writeHead(200, {
